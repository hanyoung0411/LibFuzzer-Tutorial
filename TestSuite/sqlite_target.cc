...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if( size<3 ) return 0;   // Early out for unsufficient data

  // Extract selector byte
  if( data[1]=='\n' ){
    uSelector = data[0];  data += 2; size -= 2;
  }else{// default selector byte
    uSelector = 0xfd; //0b11111101
  }
  ...
  // Bit 0 of selector : enables progress callbacks
  // Bit 1 of selector : return code from progress callbacks
  if( uSelector & 1 ){
  ...
  // Bit 2 of selector : enables foreign key constraints
  sqlite3_db_config(db, SQLITE_DBCONFIG_ENABLE_FKEY, uSelector&1, &rc);
  uSelector >>= 1;

  // Remaining bits of the selector determine a limit on the number of result rows
  execCnt = uSelector + 1;

  // Run the SQL. sqlite_exec() interface expects a zero-terminated string
